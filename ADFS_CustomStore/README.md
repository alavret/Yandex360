...
Для проверки работоспособности решения запускаем браузер (для гарантированного перехода на страницу ADFS браузер нужно запускать в режиме Incognito/InPrivate). Заранее открываем и настраиваем на этой страничке консоль разработчика.
...
Как правило, консоль разработчика открывается справа от основного окна браузера, для удобства дальнейшей работы переместите консоль  разработчика вниз экрана. Это можно сделать с помощью вот этой кнопки:

<img src="images/check_SAML_02.jpg" width="700">

После этого перейдите в консоли разработчика в раздел `Network` и установите параметр `Preserve Logs`:

<img src="images/check_SAML_03.jpg" width="900">

Далее выполняем процесс тестового входа.
- переходим на сраницу аутентификации в Яндекс 360 (например, https://mail.yandex.ru), вводим свои учётные данные. 
- система аутентификации Яндекс Паспорт перенаправит нас на ваш ADFS сервер.
- выполняем процедуру аутентификации на странице входа ADFS (вводим идентификатор пользователя и пароль). 
...
Теперь ищем нужный HTTP пакет, который содержит SAMLRespose от ADFS сервера. Для этого в консоли разработчика в закладке Network в строке фильтра вводим `commit` и выделяем мышкой четвёртый найденный пакет в трассировке.

<img src="images/check_SAML_04_1.jpg" width="900">
...
<img src="images/check_SAML_06.jpg" width="900">
...
Окно анализа SAML Resonse содержит несколько окон с различным форматом предоставления информации. Для быстрого анализа предлагается воспользоваться самым нижним разделом. Для перемотри страницы в них поместите мышку у края окна и сделайте перемотку вниз.

<img src="images/check_SAML_07.jpg" width="900">
